  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {tutors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tutors.map((tutor) => (
            <div key={tutor._id} className="bg-white rounded-xl shadow-md border border-gray-200 hover:shadow-lg">
              <img src={tutor.image || 'https://via.placeholder.com/400x300?text=Tutor+Image'} alt={tutor.language} className="w-full h-48 object-cover" />
              <div className="p-5">
                <h3 className="text-xl font-bold text-gray-800 mb-2">{tutor.language}</h3>
                <p className="text-gray-600 mb-2">{tutor.description}</p>
                <p className="text-sm text-gray-500 mb-2">Email: {tutor.email}</p>
                <div className="flex space-x-2">
                  <Link
                    to={`/edit/${tutor._id}`}
                    state={{ tutor }}
